...
export default function Blog({ id }) {
  ...
  useEffect(() => {
    ...
    setOtherBlogs(
      JSON.parse(sessionStorage.getItem("blogs")).filter(
        (x) => x.id !== parseInt(id)
      )
    );
  }, [id]);
  ...
  return (
    <div className="mt-10 md:m-20 grid grid-cols-1 gap-4 md:grid-cols-4 justify-center justify-items-center">
      <div className="blog col-span-1 md:col-span-3 space-y-3 z-50">
        <div className="flex items-center space-x-3 mx-5">
          ...
          <p className="font-semibold text-2xl lg:text-4xl text-white">
            <span className="font-extrabold">Blog:</span> {blog.title}
          </p>
        </div>
        <div className="p-5 xl:p-8 rounded-2xl bg-transparent md:bg-cyan-900 space-y-3">
      ...
      <div className="md:col-span-1 my-5 mx-5 hidden md:block">
        <p className="text-white font-semibold text-xl px-4">
        ...
        <ul className="text-white space-y-10 mt-10 mx-5">
          {otherBlogs.map((otherBlog, i) => {
            return (
              <Link
                key={i}
                className="flex items-center gap-5 bg-cyan-950 p-4 rounded-xl "
              >
                <Image
      ...
      <div className="w-full flex justify-center">
        <Link
          href="/blog"
          className=" bg-white text-xl text-black rounded-full px-4 py-2 font-semibold mb-10"
        >
